...
match_dir = os.path.join('/'.join(video_name.split('/')[:-2]))
gt_ball_dir = os.path.join(match_dir, 'ball_trajectory')

if not os.path.exists(gt_ball_dir):
    os.makedirs(gt_ball_dir)

basename = video_name.split('/')[-1][:-4]
filename = os.path.join(gt_ball_dir, basename)
